...
instance_vpn_id_a=$(terraform output -json ec2-vpn-a | jq -r '.ec2_instance_id' )
...
echo ""
echo ""
echo "Connecting to instance vpn-a [${instance_vpn_id_a}] to check ipsec tunnel"
aws ssm start-session --target "$instance_vpn_id_a" --region $region_a --document-name AWS-StartInteractiveCommand --parameters command="export AWS_PAGER=""; set -x; date; hostname -I; sudo ipsec status; sudo ip xfrm policy show; sudo ip xfrm state show" | grep -v -e "Exiting session with sessionId" -e "Starting session with SessionId:"
echo ""
echo ""
